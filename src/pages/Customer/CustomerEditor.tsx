...
import { IconAlertFill, IconHome, IconInformationFill, IconInvoice, IconProperties, IconSingleUserUnfilled } from "../../utils/SvgUtil";
                            ...
                            <div className="text-sm font-medium flex flex-col gap-4 px-2.5">
                                <div className="grid items-center gap-2.5 grid-cols-3">
                                    <div className="flex flex-col gap-1 col-span-2">
                                        ...
                                        <Select2 placeholder="Select Customer Type" accessor="customer_type" options={customerTypeSelect} buttonClass="border-gray-300" className="w-full" value={[formState.customer_type.value]} onChange={(key: string, value: (string | number)[]) => handleFormChange(key, value.length ? Number(value[0]) : 0)} />
                                    </div>
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <div className="grid grid-cols-12">
                                        <div className="col-span-2 h-full">
                                            <Select2 options={customerTitle} accessor="customer_title" value={[formState.customer_title.value]} onChange={(key: string, value: (string | number)[]) => handleFormChange(key, value.length ? value[0] : "")} buttonClass="rounded-r-none !py-2 h-full border-gray-300" className="h-full " />
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-4">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="col-span-1 flex flex-col text-sm font-medium gap-1">
                                                    ...
                                                    <Select2 accessor="customer_source" options={customerRefferalSource} value={[formState.customer_source.value]} onChange={(key: string, value: (string | number)[]) => handleFormChange(key, value.length ? value[0] : 0)} />
                                                </div>
                                                <div className="col-span-1 flex flex-col text-sm font-medium gap-1">
                                                    <span>Preferred Invoice Delivery Method</span>
                                                    <Select2 accessor="customer_invoice_delivery_preference" options={customerInvoiceDelivery} value={[formState.customer_invoice_delivery_preference.value]} onChange={(key: string, value: (string | number)[]) => handleFormChange(key, value.length ? Number(value[0]) : 0)} />
                                                </div>
                                                <div className="col-span-1 flex flex-col text-sm font-medium gap-1">
                                                    <span>Customer Invoicing Schedule</span>
                                                    <Select2 accessor="customer_when_to_invoice" options={customerInvoiceSchedule} value={[formState.customer_when_to_invoice.value]} onChange={(key: string, value: (string | number)[]) => handleFormChange(key, value.length ? Number(value[0]) : 0)} />
                                                </div>
                                                <div className="col-span-1 flex flex-col text-sm font-medium gap-1">
                                                    <span>This Customer is Tax Exempt</span>
                                                    <Select2 accessor="customer_is_tax_exempt" options={customerTaxExempt} value={[formState.customer_is_tax_exempt.value]} onChange={(key: string, value: (string | number)[]) => handleFormChange(key, value.length ? Number(value[0]) : 0)} />
                                                </div>
                                            </div>
                                            <div className="flex flex-col gap-3">
                                                <span>Preferred Communication Method</span>
                                                <div className="flex flex-col font-normal gap-2.5">
                                                    <div className="col-span-1 flex items-center cursor-pointer" onClick={() => handleFormChange('customer_is_receive_phone', (Number(formState.customer_is_receive_phone.value) + 1) % 2)} >
                                                        <Checkbox checked={Boolean(formState.customer_is_receive_phone.value)} />
                                                        <span className="text-sm">Phone Call</span>
                                                    </div>
                                                    <div className="col-span-1 flex items-center cursor-pointer" onClick={() => handleFormChange('customer_is_receive_email', (Number(formState.customer_is_receive_email.value) + 1) % 2)} >
                                                        <Checkbox checked={Boolean(formState.customer_is_receive_email.value)} />
                                                        <span className="text-sm">Email</span>
                                                    </div>
                                                    <div className="col-span-1 flex items-center cursor-pointer" onClick={() => handleFormChange('customer_is_receive_text', (Number(formState.customer_is_receive_text.value) + 1) % 2)} >
                                            ...
                                            <div className="flex flex-col gap-4">
                                                ...
                                                <div>
                                                    <HintCard description="Would you like to capture any additional client details?" linkText="Add Custom Field" />
                                                </div>
                                            </div>
                                        </div>
                                    </Accordion>
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-4 px-2.5">
                        <div className="flex p-3.5 gap-3 bg-warning-lighter rounded-xl">
                            ...
                            <div className="flex flex-col gap-1">
                                <span className="font-medium text-sm">These are default late fee rules listing. You can modify this for your customer preference.</span>
                        ...
                        <div className="flex p-3.5 gap-3 bg-warning-lighter rounded-xl">
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <span className=" text-sm text-text-dark">Late fees can be set as a dollar amount or a percentage of the invoice amount.
                                    <br></br>
                                    <br></br>
                                    *IMPORTANT - Changing late fee rules here DOES NOT change the late fee date or amount on any invoices already created. It will only change the late fee terms and amounts on newly created invoices.</span>
                            </div>
                        </div>
                        <div className="flex flex-col gap-1">
                        ...
                        <div className="grid items-center grid-cols-2 gap-4">
                            <div className="col-span-1 flex items-center cursor-pointer" onClick={() => handleFormChange('customer_outstanding', (Number(formState.customer_outstanding.value) + 1) % 2)} >
                                <Checkbox checked={Boolean(formState.customer_outstanding.value)} />
                                <span className="text-sm">Hide Outstanding Balance</span>
                            </div>
                            <div className="col-span-1 flex items-center cursor-pointer" onClick={() => handleFormChange('customer_inv_due_date', (Number(formState.customer_inv_due_date.value) + 1) % 2)}>
                                <Checkbox checked={Boolean(formState.customer_inv_due_date.value)} />
                                <span className="text-sm">Show Invoice Due Date</span>
                            </div>
                            <div className="col-span-1 cursor-pointer" onClick={() => handleFormChange('customer_total_due', (Number(formState.customer_total_due.value) + 1) % 2)}>
                                <Checkbox checked={Boolean(formState.customer_total_due.value)} />
                                <span className="text-sm">Hide Total Due on Account</span>
                            </div>
                            <div className="col-span-1 cursor-pointer" onClick={() => handleFormChange('customer_stamp_pdf_view', (Number(formState.customer_stamp_pdf_view.value) + 1) % 2)}>
                            ...
                            <div className="col-span-1 flex items-center cursor-pointer" onClick={() => handleFormChange('customer_credit_available_show', (Number(formState.customer_credit_available_show.value) + 1) % 2)}>
