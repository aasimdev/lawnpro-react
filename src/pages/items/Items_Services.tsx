...
export interface ItemsServicesData {
    id: number;
    name: string;
    cost: number;
    tax1: number;
...
const Items_Services: React.FC = () => {
    const [itemsServices, setItemsServices] = useState<ItemsServicesData[]>([]);
    const [selectedItemsServices, setSelectedItemsServices] = useState<ItemsServicesData[]>([]);
    const [isDeleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const navigate = useNavigate()

    const itemsServicesData = [{
        id: 100,
        name: "Add pine straw, Blow Off Driveways and Walkways",
        cost: 55.00,
        tax1: 0.00,
        tax2: 0.00,
        category: 1,
    ...
    const handleSelectionChange = (selectedRows: ItemsServicesData[]) => {
        setSelectedItemsServices(selectedRows);
    };
    ...
    useEffect(() => {
        ...
        setItemsServices(itemsServicesData)
    ...
    const columns: Column<ItemsServicesData>[] = [
        ...
        {
            header: 'Cost', accessor: 'cost', sortable: false,
            render: (row: ItemsServicesData) => (
                <span>{toDecimalPlaces(row.cost, 2)}</span>
            )
        },
        {
    ...
    const renderAdditionalActions = (row: ItemsServicesData) => {
        ...
        <div className="px-8 pt-20 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <DropdownMenu title="Items & Services" icon={<IconSettings size={14} color="#525866" />} items={pageTypeItem} className="bg-white text-[#525866] rounded-lg border-solid border-[1px] border-main-gray" />
            ...
            <DataTable data={itemsServicesData}
                columns={columns}
                totalPages={5}
